...
namespace openloco::windows::StationWindow
{
    ...
    // 0x0048EF02
    static void drawRatingBar(Window& w, gfx::drawpixelinfo_t& dpi, int16_t x, int16_t y, uint8_t amount, colour_t colour)
    {
        registers regs;
        regs.al = amount;
        regs.cx = x;
    ...
    // 0x0048ED2F
    void drawScroll2(Window& w, gfx::drawpixelinfo_t& dpi)
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            if (!cargo.empty())
            {
                ...
                drawRatingBar(w, dpi, 100, y, amount, colour);
