    ...
    protected:
        ...
        static TileBytes snapshotBytes(Tile tile)
        {
            ...
            for (auto& el : tile)
            {
                ...
                std::memcpy(bytes.data(), &el, kTileElementSize);
                out.push_back(bytes);
            }
            return out;
        }
...
TEST_F(TileManagerTest, BoundaryTilesAreUsable)
{
    ...
    EXPECT_EQ(a.size(), 2u);
    EXPECT_EQ(b.size(), 2u);
    EXPECT_EQ(a[1]->type(), ElementType::track);
    EXPECT_EQ(b[1]->type(), ElementType::tree);
}
...
TEST_F(TileManagerTest, DefragmentTilePeriodicCompactsTilePointerBackward)
{
    ...
    auto* addrAfter = tileAfter[0];
    EXPECT_LT(addrAfter, addrBefore);
    EXPECT_EQ(tileAfter[0]->type(), ElementType::surface);
    EXPECT_EQ(tileAfter[1]->type(), ElementType::track);
...
TEST_F(TileManagerTest, DefragmentTilePeriodicReducesElementsEnd)
{
    ...
    const auto liveBefore = TileManager::getElements().size();
    ...
    EXPECT_LT(TileManager::getElements().size(), liveBefore);
}
...
TEST_F(TileManagerTest, DisablePeriodicDefragSkipsNextCall)
{
    ...
    const auto liveBeforeDisabled = TileManager::getElements().size();

    TileManager::disablePeriodicDefrag();
    TileManager::defragmentTilePeriodic();
    EXPECT_EQ(TileManager::getElements().size(), liveBeforeDisabled);
}
...
TEST_F(TileManagerTest, GetElementsReflectsLiveRegion)
{
    ...
    for (auto& el : elements)
    {
        ...
        EXPECT_TRUE(el.isLast());
    }
    EXPECT_EQ(elements.size() + TileManager::numFreeElements(), static_cast<size_t>(TileManager::numFreeElements()) + elements.size());
}

TEST_F(TileManagerTest, GetElementsCountGrowsAfterInsert)
{
    const auto before = TileManager::getElements().size();
    TileManager::insertElement(ElementType::track, toWorldSpace(kTestTile), 8, 0);
    EXPECT_GT(TileManager::getElements().size(), before);
}

TEST_F(TileManagerTest, SetElementsRoundTrip)
{
    TileManager::insertElement(ElementType::track, toWorldSpace(kTestTile), 8, 0);
    TileManager::insertElement(ElementType::tree, toWorldSpace(kOtherTile), 16, 0);
    TileManager::reorganise();

    auto live = TileManager::getElements();
    std::vector<TileElement> snapshot(live.begin(), live.end());

    TileManager::setElements(std::span<TileElement>(snapshot.data(), snapshot.size()));
    ...
    ASSERT_EQ(b.size(), 2u);
    EXPECT_EQ(a[1]->type(), ElementType::track);
    EXPECT_EQ(a[1]->baseZ(), 8);
    EXPECT_EQ(b[1]->type(), ElementType::tree);
    EXPECT_EQ(b[1]->baseZ(), 16);
}
...
TEST_F(TileManagerTest, UpdateTilePointersRebuildsConsistently)
{
    ...
    ASSERT_EQ(a.size(), 2u);
    ASSERT_EQ(b.size(), 2u);
    EXPECT_EQ(a[0]->type(), ElementType::surface);
    EXPECT_EQ(a[1]->type(), ElementType::track);
    EXPECT_EQ(b[0]->type(), ElementType::surface);
    EXPECT_EQ(b[1]->type(), ElementType::tree);
}
...
TEST_F(TileManagerTest, InsertElementRoadBasicAddsRoadElement)
{
    ...
    auto tile = TileManager::get(kTestTile);
    ASSERT_EQ(tile.size(), 2u);
    EXPECT_EQ(tile[0]->type(), ElementType::surface);
    EXPECT_EQ(tile[1]->type(), ElementType::road);
}

TEST_F(TileManagerTest, InsertElementRoadStopsBeforeRoadStationAtSameBaseZ)
{
    auto* station = TileManager::insertElement<StationElement>(toWorldSpace(kTestTile), 16, 0);
    ASSERT_NE(station, nullptr);
    station->setStationType(OpenLoco::StationType::roadStation);
    ...
    EXPECT_EQ(tile[0]->type(), ElementType::surface);
    EXPECT_EQ(tile[1]->type(), ElementType::road);
    EXPECT_EQ(tile[2]->type(), ElementType::station);
}
...
TEST_F(TileManagerTest, TileSurfaceReturnsTheSurfaceElement)
{
    ...
    EXPECT_EQ(surface->type(), ElementType::surface);
    EXPECT_EQ(surface->baseZ(), 4);
    EXPECT_EQ(static_cast<TileElementBase*>(surface), tile[0]);
}
...
TEST_F(TileManagerTest, TileTrainStationFindsStationFollowingMatchingTrack)
{
    ...
    ASSERT_NE(anchor, nullptr);
    auto* station = TileManager::insertElementAfterNoReorg<StationElement>(anchor, toWorldSpace(kTestTile), 8, 0);
    ASSERT_NE(station, nullptr);
