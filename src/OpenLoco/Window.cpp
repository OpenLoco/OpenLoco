...
namespace OpenLoco::Ui
{
    ...
    void window::viewportsUpdatePosition()
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            if (config->viewport_target_sprite != 0xFFFF)
            {
                ...
                int z = (TileManager::get(thing->x, thing->y).getHeight().landHeight) - 16;
    ...
    void window::viewportGetMapCoordsByCursor(int16_t* map_x, int16_t* map_y, int16_t* offset_x, int16_t* offset_y)
    {
        ...
        *map_y = interaction.y;
        ...
        int32_t base_height = TileManager::get(*map_x, *map_y).getHeight().landHeight;
    ...
    void window::viewportCentreOnTile(const Map::map_pos3& loc)
    {
        ...
        auto tileHeight = TileManager::get(loc.x, loc.y).getHeight().landHeight;
        tileHeight -= 16;
