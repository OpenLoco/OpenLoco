...
namespace OpenLoco
{
    ...
    private:
        ...
        inline static loco_global<industry_id_t[max_cargo_stats], 0x0112C7D2> getIndustryArguments;
    ...
    public:
        ...
        {
            std::fill_n(getIndustryArguments.get(), max_cargo_stats, IndustryId::null);
        }

        industry_id_t getIndustry(const uint8_t cargo) const
        {
            return getIndustryArguments[cargo];
        }

        void setIndustry(const uint8_t cargo, const industry_id_t id)
        {
            getIndustryArguments[cargo] = id;
        }
    };
