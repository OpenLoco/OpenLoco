...
namespace OpenLoco::Gfx
{
    ...
    HardwareDrawingContext::HardwareDrawingContext()
        : _state{ std::make_unique<HardwareDrawingContextState>() }
    {
    }
    ...
    void HardwareDrawingContext::clear(uint32_t fill)
    {
        auto& rt = currentRenderTarget();
        return Impl::clear(rt, fill);
    }

    void HardwareDrawingContext::clearSingle(uint8_t paletteId)
    {
        auto& rt = currentRenderTarget();
        return Impl::clearSingle(rt, paletteId);
    }

    void HardwareDrawingContext::fillRect(int16_t left, int16_t top, int16_t right, int16_t bottom, uint8_t colour, RectFlags flags)
    {
        auto& rt = currentRenderTarget();
        return Impl::fillRect(rt, left, top, right, bottom, colour, flags);
    }

    void HardwareDrawingContext::drawRect(int16_t x, int16_t y, uint16_t dx, uint16_t dy, uint8_t colour, RectFlags flags)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawRect(rt, x, y, dx, dy, colour, flags);
    }

    void HardwareDrawingContext::fillRectInset(int16_t left, int16_t top, int16_t right, int16_t bottom, AdvancedColour colour, RectInsetFlags flags)
    {
        auto& rt = currentRenderTarget();
        return Impl::fillRectInset(rt, left, top, right, bottom, colour, flags);
    }

    void HardwareDrawingContext::drawRectInset(int16_t x, int16_t y, uint16_t dx, uint16_t dy, AdvancedColour colour, RectInsetFlags flags)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawRectInset(rt, x, y, dx, dy, colour, flags);
    }

    void HardwareDrawingContext::drawLine(const Ui::Point& a, const Ui::Point& b, PaletteIndex_t colour)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawLine(rt, a, b, colour);
    }

    void HardwareDrawingContext::drawImage(int16_t x, int16_t y, uint32_t image)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawImage(&rt, x, y, image);
    }

    void HardwareDrawingContext::drawImage(const Ui::Point& pos, const ImageId& image)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawImage(rt, pos, image);
    }

    void HardwareDrawingContext::drawImageMasked(const Ui::Point& pos, const ImageId& image, const ImageId& maskImage)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawImageMasked(rt, pos, image, maskImage);
    }

    void HardwareDrawingContext::drawImageSolid(const Ui::Point& pos, const ImageId& image, PaletteIndex_t paletteIndex)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawImageSolid(rt, pos, image, paletteIndex);
    }

    void HardwareDrawingContext::drawImagePaletteSet(const Ui::Point& pos, const ImageId& image, PaletteMap::View palette, const G1Element* noiseImage)
    {
        auto& rt = currentRenderTarget();
        return Impl::drawImagePaletteSet(rt, pos, image, palette, noiseImage);
    }

    void HardwareDrawingContext::pushRenderTarget(const RenderTarget& rt)
    {
    ...
    void HardwareDrawingContext::popRenderTarget()
    {
    ...
    const RenderTarget& HardwareDrawingContext::currentRenderTarget() const
    {
        // Should not be empty.
        assert(_state->rtStack.empty() == false);
