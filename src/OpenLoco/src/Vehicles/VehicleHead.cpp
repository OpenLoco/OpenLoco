...
namespace OpenLoco::Vehicles
{
        ...
        else
        {
            ...
            for (auto& handle : ring)
            {
                ...
                const auto hasSignal = routing & World::Track::AdditionalTaDFlags::hasSignal;
                {
                    ...
                    updateSignalOccupiedInBlock(pos, signaledTad, companyId, trackObjId);
                }
                if (handle != veh2.routingHandle)
                {
                    if (hasSignal)
    ...
    bool positionVehicleOnTrack(VehicleHead& head, const bool isPlaceDown)
    {
        ...
        if (head.mode != TransportMode::road)
        {
            ...
            updateSignalOccupiedInBlock(reversePos, reverseTad, head.owner, head.trackType);
            updateSignalOccupiedInBlock(pos, tad, head.owner, head.trackType);
        }
        return unkFlag;
    }
}
