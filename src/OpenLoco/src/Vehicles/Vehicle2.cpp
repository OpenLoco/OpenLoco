...
namespace OpenLoco::Vehicles
{
    ...
    static bool shouldSetVar5E(const Vehicle& train, VehicleBogie& frontBogie)
    {
        ...
        if (train.head->hasFlags(EntityBaseFlags::manualControl) && train.head->var_6E <= 10)
        {
            return false;
        }
    ...
    bool Vehicle2::update()
    {
        ...
        if (!(*_vehicleUpdate_head)->hasFlags(EntityBaseFlags::manualControl))
        {
        ...
        {
            ebp /= 2;
            if (!train.head->hasFlags(EntityBaseFlags::unk_0))
            {
                var_5A = 4;
        ...
        {
            if (train.head->hasFlags(EntityBaseFlags::manualControl))
            {
        ...
        if (speedAdjustment < 0.0_mph)
        {
            ...
            if (train.head->hasFlags(EntityBaseFlags::manualControl) || !isOnRackRail)
            {
                minSpeed = 0.0_mph;
            }
