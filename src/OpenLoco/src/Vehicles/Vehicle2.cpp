...
namespace OpenLoco::Vehicles
{
    ...
    static bool shouldSetVar5E(const Vehicle& train, VehicleBogie& frontBogie)
    {
        ...
        if (train.head->hasEntityBaseFlags(EntityBaseFlags::manualControl) && train.head->var_6E <= 10)
        {
            return false;
        }
    ...
    bool Vehicle2::update()
    {
        ...
        if (!(*_vehicleUpdate_head)->hasEntityBaseFlags(EntityBaseFlags::manualControl))
        {
        ...
        {
            ebp /= 2;
            if (!train.head->hasEntityBaseFlags(EntityBaseFlags::unk_0))
            {
                var_5A = 4;
        ...
        {
            if (train.head->hasEntityBaseFlags(EntityBaseFlags::manualControl))
            {
        ...
        if (speedAdjustment < 0.0_mph)
        {
            ...
            if (train.head->hasEntityBaseFlags(EntityBaseFlags::manualControl) || !isOnRackRail)
            {
                minSpeed = 0.0_mph;
            }
