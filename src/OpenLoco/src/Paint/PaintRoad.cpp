...
#include "Paint/PaintRoadAdditionsData.h"
#include "Paint/PaintRoadCommonData.h"
#include "Paint/PaintRoadStyle0Data.h"
#include "Paint/PaintRoadStyle1Data.h"
#include "Paint/PaintRoadStyle2Data.h"
...
namespace OpenLoco::Paint
{
    ...
    static void paintLevelCrossing(PaintSession& session, const ImageId baseRoadImageColour, const World::RoadElement& elRoad, const uint8_t rotation)
    {
        ...
        if (frame != 0)
        {
            if (frame == 15)
            {
                frame = (((ScenarioManager::getScenarioTicks() / (1U << crossingObj->closedAnimationSpeed)) & (crossingObj->closedAnimationFrames - 1)) + crossingObj->closingAnimationFrames + 1);
            }
        }
