...
namespace OpenLoco::IndustryManager
{
    ...
    static std::optional<World::Pos2> findRandomNewIndustryLocation(const uint8_t indObjId)
    {
        ...
        for (auto i = 0; i < kFindRandomNewIndustryAttempts; ++i)
        {
            ...
            // Map::Pos2 randomPos{
            //     Map::TilePos2(gPrng1().randNext(Map::getMapRows()), gPrng1().randNext(Map::getMapColumns()))
            // };
            const auto randomNum = gPrng1().randNext();

            const auto randomPos = World::toWorldSpace(World::TilePos2((((randomNum >> 16) * World::TileManager::getMapRows()) >> 16), (((randomNum & 0xFFFF) * World::TileManager::getMapColumns()) >> 16)));
