...
#pragma pack(pop)
    ...
    namespace Common
    {
        ...
        template<typename T, int LegacyCapacity>
        void copyToLegacyList(const T& sflType, uint8_t (&legacyList)[LegacyCapacity])
        {
            static_assert(LegacyCapacity >= sflType.capacity());
