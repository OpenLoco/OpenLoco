...
namespace OpenLoco::Ui::Windows::ToolbarBottom::Editor
{
    ...
    static void draw(Window& self, Gfx::RenderTarget* rt)
    {
        ...
        if (EditorController::canGoBack())
        {
            drawingCtx.drawRectInset(*rt, previous.left + self.x + 1, previous.top + self.y + 1, previous.width() - 2, previous.height() - 2, self.getColour(WindowColour::secondary), 0x30);
        }
        drawingCtx.drawRectInset(*rt, next.left + self.x + 1, next.top + self.y + 1, next.width() - 2, next.height() - 2, self.getColour(WindowColour::secondary), 0x30);
