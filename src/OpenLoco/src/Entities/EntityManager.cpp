...
namespace OpenLoco::EntityManager
{
    ...
    void moveSpatialEntry(EntityBase& entity, const World::Pos3& loc)
    {
        ...
        if (newIndex != oldIndex)
        {
            ...
            {
                Diagnostics::info("Invalid quadrant ids... Reseting spatial index.");
                resetSpatialIndex();
    ...
    static EntityBase* createEntity(EntityId id, EntityListType list)
    {
        ...
        {
            Diagnostics::error("Tried to create invalid entity! id: {}, list: {}", enumValue(id), enumValue(list));
            return nullptr;
        }
    ...
    void moveEntityToList(EntityBase* const entity, const EntityListType list)
    {
        ...
        else
        {
            ...
            {
                Diagnostics::error("Invalid previous entity id. Entity linked list corrupted? Id: {}", enumValue(previousId));
            }
            else
            {
        ...
        if (nextId != EntityId::null)
        {
            ...
            {
                Diagnostics::error("Invalid next entity id. Entity linked list corrupted? Id: {}", enumValue(nextId));
            }
            else
            {
        ...
        if (entity->nextThingId != EntityId::null)
        {
            ...
            if (nextEntity == nullptr)
            {
                Diagnostics::error("Invalid next entity id. Entity linked list corrupted? Id: {}", enumValue(entity->nextThingId));
            }
            else
            {
