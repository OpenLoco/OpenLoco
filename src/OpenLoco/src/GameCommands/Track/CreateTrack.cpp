...
namespace OpenLoco::GameCommands
{
    ...
    static uint32_t createTrack(const TrackPlacementArgs& args, uint8_t flags)
    {
        ...
        for (auto& piece : trackPieces)
        {
            ...
            newElTrack->setHasBridge(getLegacyReturnState().byte_1136073 & (1U << 1));
        ...
        if (getLegacyReturnState().byte_1136073 & (1U << 0))
        {
            auto* bridgeObj = ObjectManager::get<BridgeObject>(args.bridge);
        ...
        if ((getLegacyReturnState().byte_1136072 & ElementPositionFlags::underground) != ElementPositionFlags::none)
        {
