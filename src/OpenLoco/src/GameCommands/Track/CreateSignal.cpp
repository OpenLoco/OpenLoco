            ...
            if (!(flags & (Flags::aiAllocated | Flags::ghost)))
            {
                const uint16_t tad = args.rotation | (args.trackId << 3);
                {
                    ...
                    if (!tc.connections.empty())
                    {
                        ...
                        Vehicles::updateSignalOccupiedInBlock(nextLoc, tad2, getUpdatingCompanyId(), args.trackObjType);
                    }
                }
                ...
                if (!tc.connections.empty())
                {
                    Vehicles::TrackAndDirection::_TrackAndDirection tad2{ 0, 0 };
                    tad2._data = tc.connections.front() & World::Track::AdditionalTaDFlags::basicTaDWithSignalMask;
                    Vehicles::updateSignalOccupiedInBlock(nextTrackStart, tad2, getUpdatingCompanyId(), args.trackObjType);
                }
            }
        }
        return totalCost;
