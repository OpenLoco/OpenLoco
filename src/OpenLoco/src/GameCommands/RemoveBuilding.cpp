    ...
    static uint32_t removeBuilding(Map::Pos3& pos, uint8_t flags)
    {
        ...
        for (auto& el : tile)
        {
            ...
            if (!isEditorMode())
            {
                ...
                {
                    if ((buildingObj->flags & BuildingObjectFlags::undestructible) != BuildingObjectFlags::none)
                    {
            ...
            auto animOffsets = getBuildingTileOffsets((buildingObj->flags & BuildingObjectFlags::largeTile) != BuildingObjectFlags::none);
