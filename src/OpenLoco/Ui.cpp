...
#pragma pack(pop)
	...
	static void processMouseTool(int16_t x, int16_t y)
	{
		...
		auto toolWindow = WindowManager::find(WindowManager::getToolWindowType(), WindowManager::getToolWindowNumber());
		if (toolWindow != nullptr)
		{
			toolWindow->callToolUpdate(WindowManager::getToolWidgetIndex(), x, y);
		}
		else
		{
			Input::toolCancel();
