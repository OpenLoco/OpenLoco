...
// amount : ebx
static MoneyEffect* createEffect(const Map::map_pos3& loc, const company_id_t company, const currency32_t amount)
{
    if (isTitleMode())
    {
        return nullptr;
...
// company : updatingCompanyId global
// amount : ebx
MoneyEffect* OpenLoco::MoneyEffect::create(const Map::map_pos3& loc, company_id_t company, currency32_t amount)
{
    if (isEditorMode())
    {
        return nullptr;
    ...
    return createEffect(pos, company, -amount);
}
