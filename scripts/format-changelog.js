...
const ChangeTypes = ["Headline feature", "Feature", "Improved", "Change", "Removed", "Fix", "Technical"];
...
function readVersionHeader(ctx) {
    ...
    const versionMatch = versionLine.text.match(/^((\d+\.\d+(\.\d+)?(\+|-\w+)?) \((\d{4}-\d{2}-\d{2}|\?\?\?)\))$/);
...
function cleanupVersionBlock(version) {
    ...
    version.entries.forEach(entry => {
        ...
        var originalBug = false;
        if (desc.endsWith("(original bug)")) {
            originalBug = true;
            entry.description = desc.substring(0, desc.indexOf("(original bug")).trim();
        }
